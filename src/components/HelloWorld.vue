...
const fetchData = async () => {
  const data = await state.stateGenaratorApi({
    ...
    StateStore: 'users11',
    reqs: {
      url: 'v3/patients?Rows=100&status=1&page=1&both=1&branchId=1', method: 'get', data: {},
    }, //data:{sex:male} is not required
    config:{
      headers: {
        "X-request_date_client": Date.now(),
        "X-app_key": "MERCHANT_MANAGE_WEB_PC",
        "X-token_id": "7867687687 whkaj adjk",
        "X-owner_id": "7867687687 whkaj adjk",
        "X-owner_type": "7867687687 whkaj adjk"
      }
    },
